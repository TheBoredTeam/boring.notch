...
class StampVersionTests(unittest.TestCase):
    def test_stamps_all_project_versions_and_build_numbers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Semver build metadata must be quoted to stay valid PBX syntax.
            stamp_pbxproj(pbxproj, "2.8.0+build.1", "274")

            contents = pbxproj.read_text(encoding="utf-8")
            self.assertEqual(contents.count('MARKETING_VERSION = "2.8.0+build.1";'), 2)
            self.assertEqual(contents.count("CURRENT_PROJECT_VERSION = 274;"), 2)
    ...
    def test_build_number_validation(self) -> None:

...
def quote_pbx_value(value: str) -> str:
    """Return a PBX string literal with characters escaped safely."""
...
def stamp_pbxproj(pbxproj_path: Path, version: str | None, build_number: str) -> None:
    ...
    if version:
        text, version_count = replace_build_setting(
            text, "MARKETING_VERSION", quote_pbx_value(version)
        )
        if version_count == 0:

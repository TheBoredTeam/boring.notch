...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("spruce.notch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: SpruceViewModel] = [:] // UUID -> SpruceViewModel
    var window: NSWindow?
    let vm: SpruceViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? SpruceNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? SpruceNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createSpruceNotchWindow(for screen: NSScreen, with viewModel: SpruceViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = SpruceNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func playWelcomeSound() {
        ...
        audioPlayer.play(fileName: "spruce", fileExtension: "m4a")
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = SpruceViewModel(screenUUID: uuid)
                    let window = createSpruceNotchWindow(for: screen, with: viewModel)

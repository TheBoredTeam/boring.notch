...
struct WeatherSnapshot {
    ...
    let hourlyForecast: [WeatherHourlyPoint]
    ...
    var highLowText: String? {
        guard let highTemperature, let lowTemperature else { return nil }
        return "H \(Int(highTemperature.rounded()))°  L \(Int(lowTemperature.rounded()))°"
    }
...
final class WeatherManager: ObservableObject {
    ...
    private func makeWeatherSnapshot(
        ...
        let hourlyForecast = buildHourlyForecast(
            from: forecast.hourly,
            currentTime: current.time,
            fallbackIsDay: current.isDay == 1,
            timeZone: weatherTimeZone
        )

        let today = dailyForecast.first
        let sunrise = forecast.daily?.sunrise?.first.flatMap { parseDateTimeString($0, timeZone: weatherTimeZone) }
        let sunset = forecast.daily?.sunset?.first.flatMap { parseDateTimeString($0, timeZone: weatherTimeZone) }
    ...
    private func buildHourlyForecast(
        ...
        let precipitation = hourly.precipitationProbability ?? []
        let count = min(times.count, temperatures.count)
        guard count > 0 else { return [] }

        let startIndex: Int = {
            if let currentTime, let idx = times.firstIndex(of: currentTime) {
                return idx
            }
            if let idx = times.firstIndex(where: {
                guard let date = parseDateTimeString($0, timeZone: timeZone) else { return false }
                return date >= Date()
            }) {
                return idx
            }
        ...
        let endIndex = min(count, startIndex + 8)
        guard startIndex < endIndex else { return [] }

        return (startIndex..<endIndex).map { index in
            let rawTime = times[index]
            let timeLabel = index == startIndex
                ? l10n("weather_relative_now", fallback: "Now")
                : hourLabel(from: rawTime, timeZone: timeZone)
            let code = index < codes.count ? codes[index] : 0
            let rain = index < precipitation.count ? precipitation[index] : nil

            return WeatherHourlyPoint(
                id: rawTime,
                timeLabel: timeLabel,
                temperature: temperatures[index],
                weatherCode: code,
                isDay: inferIsDay(from: rawTime, fallback: fallbackIsDay, timeZone: timeZone),
    ...
    private func buildDailyForecast(
        ...
        return (0..<count).map { index in
            ...
            return WeatherDailyPoint(
                ...
                dateLabel: parsedDate.map { monthDayFormatter(for: timeZone).string(from: $0) } ?? dateRaw,
                weatherCode: code,
    ...
    private func hourLabel(from rawTime: String, timeZone: TimeZone) -> String {
        if let parsed = parseDateTimeString(rawTime, timeZone: timeZone) {
            return hourFormatter(for: timeZone).string(from: parsed)
        }
        if let value = rawTime.split(separator: "T").last {
    ...
    private func inferIsDay(from rawTime: String, fallback: Bool, timeZone: TimeZone) -> Bool {
        guard let date = parseDateTimeString(rawTime, timeZone: timeZone) else { return fallback }
        let hour = calendar(for: timeZone).component(.hour, from: date)
    ...
    private func parseDateTimeString(_ raw: String, timeZone: TimeZone) -> Date? {
        dayTimeFormatter(for: timeZone).date(from: raw)
    }
    ...
    private func dayTimeFormatter(for timeZone: TimeZone) -> DateFormatter {
        formatter(dateFormat: "yyyy-MM-dd'T'HH:mm", locale: Locale(identifier: "en_US_POSIX"), timeZone: timeZone)
    }

    private func hourFormatter(for timeZone: TimeZone) -> DateFormatter {
        formatter(dateFormat: "HH:mm", locale: Locale.current, timeZone: timeZone)
    }

    private func monthDayFormatter(for timeZone: TimeZone) -> DateFormatter {
        formatter(dateFormat: "MM.dd", locale: Locale.current, timeZone: timeZone)
    }

...
struct BoringHeader: View {
    ...
    /// go. The shelf's own visibility rules still gate the shelf tab; capture
    /// is an independent second destination.
    private var showsTabBar: Bool {
        let shelfTabVisible = (!shelfState.isEmpty || coordinator.alwaysShowTabs) && Defaults[.boringShelf]
        return shelfTabVisible || Defaults[.captureEnabled]
    }

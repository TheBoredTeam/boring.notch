...
enum AgentTarget: String, Codable, CaseIterable, Identifiable, Defaults.Serializable {
    case claudeCode
    case codex
    case custom
    ...
    /// Sensible default clipboard mode per agent. Both read a pasted file path
    /// natively, so default to a path payload.

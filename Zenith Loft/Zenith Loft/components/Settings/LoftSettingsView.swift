...
struct LoftSettingsView: View {
    @StateObject var extensionManager = LoftExtensionManager()
            ...
            Group {
                ...
                case "About":
                    if let controller = updaterController {
                        LoftAbout(updaterController: controller)
                    } else {
...
struct LoftGeneralSettings: View {
...
struct LoftDownloads: View {
    @Default(.selectedDownloadIndicatorStyle) var selectedDownloadIndicatorStyle
    @Default(.selectedDownloadIconStyle) var selectedDownloadIconStyle
    var body: some View {
        Form {
            loftWarningBadge("We don't support downloads yet", "It will be supported later on.")
            Section {
...
struct LoftMedia: View {
    ...
    var body: some View {
        Form {
            Section {
                ...
                if LoftMusicManager.shared.isNowPlayingDeprecated {
                    HStack {
    ...
    private var availableMediaControllers: [LoftMediaControllerType] {
        if LoftMusicManager.shared.isNowPlayingDeprecated {
            return LoftMediaControllerType.allCases.filter { $0 != .nowPlaying }
        } else {
            return LoftMediaControllerType.allCases
        }
    }
}

struct LoftCalendarSettings: View {
...
struct LoftExtensions: View {
    @EnvironmentObject var extensionManager: LoftExtensionManager
        ...
        Form {
            ...
            Section {
                ...
                HStack {
                    ...
                    loftCustomBadge(text: "Coming soon")
                }
            }
            ...
            Section {
                Defaults.Toggle("Enable Loft mirror", key: .showMirror)
...
// MARK: - Badges / Helpers (Loft-prefixed)

func loftProFeatureBadge() -> some View {
    Text("Upgrade to Pro")
...
func loftComingSoonTag() -> some View {
    Text("Coming soon")
        .foregroundStyle(.secondary)
        .font(.footnote.bold())
        .padding(.vertical, 3)
...
func loftCustomBadge(text: String) -> some View {
    Text(text)
        .foregroundStyle(.secondary)
        .font(.footnote.bold())
        .padding(.vertical, 3)
...
func loftWarningBadge(_ text: String, _ description: String) -> some View {
    Section {
        HStack(spacing: 12) {

...
struct LoftCameraPreviewView: View {
    @EnvironmentObject var vm: LoftViewModel
    @ObservedObject var webcamManager: LoftWebcamManager
        ...
        GeometryReader { geometry in
            ZStack {
                if let previewLayer = webcamManager.previewLayer {
                    LoftCameraPreviewLayerView(previewLayer: previewLayer)
                        .scaleEffect(x: -1, y: 1)
                        .clipShape(
                            RoundedRectangle(
    ...
    private func handleCameraTap() {
        if isRequestingAuthorization { return } // Prevent multiple authorization requests
...
struct LoftCameraPreviewLayerView: NSViewRepresentable {
...
#Preview {
    LoftCameraPreviewView(webcamManager: .shared)

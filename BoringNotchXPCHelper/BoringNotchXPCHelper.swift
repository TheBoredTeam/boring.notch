...
class BoringNotchXPCHelper: NSObject, BoringNotchXPCHelperProtocol {
    ...
    // This is a write-based capability probe, not a read-only discovery API.
    // The client requests selection only while built-in brightness is enabled.
    ...
    private func ioServiceFor(displayID: CGDirectDisplayID) -> io_service_t? {
        ...
        var matchingService: io_service_t?
        while case let service = IOIteratorNext(iterator), service != 0 {
            ...
            if let vendorID = info[kDisplayVendorID] as? UInt32,
               ...
               productID == CGDisplayModelNumber(displayID) {
                // Vendor/model does not identify a particular monitor when
                // identical displays are attached. Never choose an arbitrary one.
                if let matchingService {
